...
public class SerializableDictionaryPropertyDrawer : PropertyDrawer
{
    ...
    //static GUIContent s_iconUp = IconContent("Toolbar Up", "Up entry");
    //static GUIContent s_iconDown = IconContent("Toolbar Down", "Down entry");
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		if (property.isExpanded)
		{
			...
			linePosition.xMax -= buttonWidth*3;
			...
			foreach(var entry in EnumerateEntries(keyArrayProperty, valueArrayProperty))
			{
                ...
                buttonPosition.x = linePosition.xMax + buttonWidth;      
                if (i> 0 && GUI.Button(buttonPosition, "↑"))
                {
                    buttonAction = Action.Up;
                    buttonActionIndex = i;
                }
                buttonPosition.x = linePosition.xMax + buttonWidth+ buttonWidth;
                if (i < keyArrayProperty.arraySize-1 && GUI.Button(buttonPosition, "↓"))
                {
                    buttonAction = Action.Down;
                    buttonActionIndex = i;
                }
        ...
        else if (buttonAction == Action.Up)
        {
            keyArrayProperty.MoveArrayElement(buttonActionIndex, buttonActionIndex - 1);
            valueArrayProperty.MoveArrayElement(buttonActionIndex, buttonActionIndex - 1);
        }
        else if (buttonAction == Action.Down)
        {
            keyArrayProperty.MoveArrayElement(buttonActionIndex, buttonActionIndex + 1);
            valueArrayProperty.MoveArrayElement(buttonActionIndex, buttonActionIndex + 1);
        }
